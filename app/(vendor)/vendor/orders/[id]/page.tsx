...
import { Package, MapPin, Truck } from 'lucide-react'
import VendorShipmentForm from '@/app/components/vendor/VendorShipmentForm'
import StoreInitializer, { PopulatedSerializedShipment } from '@/app/components/vendor/StoreInitializer'
import { IShipment, IOrder } from '@/app/types'
...
export default async function VendorShipmentPage({ params }: PageProps) {
  ...
  // Find the shipment and populate order fields
  const shipmentDoc = await Shipment.findOne({ _id: id }).populate('order')
  if (!shipmentDoc) notFound()

  // Parse using our explicit, non-any composite type structure
  const shipment = JSON.parse(JSON.stringify(shipmentDoc)) as PopulatedSerializedShipment
  ...
  return (
    <div className="px-2 py-2">
      {/* Hydrate global state safely with strict types */}
      <StoreInitializer shipment={shipment} />
