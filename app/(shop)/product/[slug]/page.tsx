...
export default function ProductDetailsClient({
  ...
  const cartItem = cart.find(
    (item) => item.id === (product._id as unknown as string),
  )
  ...
  const handleQuantityChange = (newQty: number) => {
    const val = Math.max(1, newQty)
    if (cartItem) {
      updateQuantity(product._id as unknown as string, val)
    } else {
      setLocalQuantity(val)
    }
  }
  ...
  const handleAddToCart = () => {
    if (cartItem) {
      ...
      addToCart({
        ...
        name: product.name,
        price: currentPrice,
        image: typeof product.mainImage === 'string' ? product.mainImage : '',
        quantity: localQuantity,
        slug: product.slug,
        brand: product.brand,
      })
          ...
          <Table withTableBorder>
            <Table.Tbody>
              <Table.Tr>
                <Table.Th bg="gray.0">Category</Table.Th>
                <Table.Td>
                  {(product.category as unknown as ICategory).name || 'General'}
                </Table.Td>
              </Table.Tr>
